...
function prompt(c) {
  return [
    ...
    `4. COMMENTS. \`docs/COMMENT_STANDARD.md\` is the rule: a comment survives only in the shape "<non-obvious fact> so that <consequence>". A comment that narrates history, defends a choice, or restates what the code plainly does is a finding — as is a comment whose claim is now FALSE, which no linter can see because its identifiers still resolve. Do not, however, expect the code to explain itself to you: the reasoning behind a change lives in its commit message, and its absence from the source is correct.`,
    `5. YOU ARE MAKING AN ARGUMENT, NOT ISSUING AN INSTRUCTION. Every finding will be adjudicated: taken, rejected as wrong, or rejected as outweighed by something you could not see. Write to convince a reader who may decide against you — state the failure concretely enough to be checked, and do not pad severity to force a fix. A finding rejected with a reason is a successful round.`,
    `6. If a genuine attempt to break it found nothing, set nothingFound=true and describe what you checked in \`checked\`. An honest empty round is a useful signal; a manufactured finding is worse than silence.`,
  ].join(' ')
}
